...
const PGversion = "1.0.0-20190901"

var localAddr = make(map[string]string)
var filter string
var (
	iface      = flag.String("i", getEnvStr("HTTPLOG_DEVICE", "eth0"), "Interface to read from")
	cpuprofile = flag.String("cpuprofile", "", "If non-empty, write CPU profile here")
	cpuNo      = flag.Int("cpu", getEnvInt("HTTPLOG_CPUNO", runtime.NumCPU()), "Number of HTTP parsing goroutine")
	snaplen    = flag.Int("s", getEnvInt("HTTPLOG_SNAPLEN", 1560), "Snaplen, if <= 0, use 65535")
	bufferSize = flag.Int("b", getEnvInt("HTTPLOG_BUFSIZE", 8), "Interface buffersize (MB)")
	//filter     = flag.String("f", getEnvStr("HTTPLOG_FILTER", "port 80 and tcp"), "BPF filter")
	...
	logdir   = flag.String("d", getEnvStr("HTTPLOG_DIR", "/log/httplog"), "Write directory for log file")
	httpPort = flag.Int("p", getEnvInt("HTTPLOG_PORT", 80), "http service port")
