...
func logSetup(logDir string) {
	...
	var logFile string
	if *logSplit == true {
		logFile = filepath.Join(logDir, "http2-CQ-"+time.Unix(0, nanos).Format("20060102")+".log")
	} else {
		logFile = filepath.Join(logDir, "http2-"+time.Unix(0, nanos).Format("20060102")+".log")
	}
	//logFile := fmt.Sprintf("%s/http2-%s.log", logDir, time.Unix(0, nanos).Format("20060102"))
	...
	go func() {
		...
		if *logSplit == false {
			...
			logFile = filepath.Join(logDir, "http2-CR-"+time.Unix(0, nanos).Format("20060102")+".log")
			f[2], _ = os.OpenFile(logFile, os.O_CREATE|os.O_WRONLY|os.O_APPEND, os.ModeAppend|0600)
			logFile = filepath.Join(logDir, "http2-SQ-"+time.Unix(0, nanos).Format("20060102")+".log")
			f[3], _ = os.OpenFile(logFile, os.O_CREATE|os.O_WRONLY|os.O_APPEND, os.ModeAppend|0600)
			logFile = filepath.Join(logDir, "http2-SR-"+time.Unix(0, nanos).Format("20060102")+".log")
